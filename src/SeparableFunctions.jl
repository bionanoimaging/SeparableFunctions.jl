...
using NDTools
export separable_view, create_separable
...
function calculate_separables(::Type{AT}, fct, sz::NTuple{N, Int}, args...) where {AT, N}
...
```
"""
function separable_view(::Type{TA}, fct, sz::NTuple{N, Int}, args...) where {TA, N}
    res = calculate_separables(TA, fct, sz, args...)
    return (.*(res...) for n=1:1) # to prevent premature evaluation
end

function separable_view(fct, sz::NTuple{N, Int}, args...) where {N}
    separable_view(Array{Float64}, fct, sz::NTuple{N, Int}, args...)
end

function create_separable(::Type{TA}, fct, sz::NTuple{N, Int}, args...) where {TA, N}
    res = calculate_separables(TA, fct, sz, args...)
    .*(res...)
end

function create_separable(fct, sz::NTuple{N, Int}, args...) where {N}
    res = calculate_separables(Array{Float64}, fct, sz, args...)
    .*(res...)
end
